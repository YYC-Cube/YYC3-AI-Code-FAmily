...
export function DesignerProvider({ children }: { children: ReactNode }) {
  ...
  const handleSetCurrentUserIdentity = useCallback((identity: CRDTUserIdentity | null) => {
  ...
  const handleLockPanel = useCallback((panelId: string) => {
  ...
  const handleUnlockPanel = useCallback((panelId: string) => {
    const identity = currentUserIdentity;
    if (!identity) return;
    setCRDTDocVersion(prev => prev + 1);
  ...
  const value: DesignerContextType = {
    ...
    // Phase 12 — RBAC 身份与面板锁定
    currentUserIdentity,
    setCurrentUserIdentity: handleSetCurrentUserIdentity,
    lockPanel: handleLockPanel,
    unlockPanel: handleUnlockPanel,
  };
