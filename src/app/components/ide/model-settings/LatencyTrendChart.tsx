...
export function LatencyTrendChart({ diagnostics }: { diagnostics: Record<string, DiagnosticResult> }) {
  ...
  const perfData = useMemo(() => {
    try {
      ...
      return parsed.filter(p => p.latencyMs != null && p.latencyMs > 0).slice(-30).map(p => ({
      ...
      <div className="h-[130px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
            ...
            <XAxis dataKey="time" tick={{ fill: 'rgba(255,255,255,0.15)', fontSize: 9 }} axisLine={{ stroke: 'rgba(255,255,255,0.06)' }} tickLine={false} />
            <YAxis tick={{ fill: 'rgba(255,255,255,0.15)', fontSize: 9 }} axisLine={{ stroke: 'rgba(255,255,255,0.06)' }} tickLine={false} unit="ms" />
            <RTooltip contentStyle={{ background: '#0d1117', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', fontSize: '10px', color: 'rgba(255,255,255,0.7)' }} labelStyle={{ color: 'rgba(255,255,255,0.4)', fontSize: '9px' }} formatter={(value: unknown) => [`${value}ms`, '延迟']} />
            <Area type="monotone" dataKey="latency" stroke="#06b6d4" strokeWidth={1.5} fill="url(#latencyGrad)" dot={{ fill: '#06b6d4', r: 2, strokeWidth: 0 }} activeDot={{ r: 3, fill: '#06b6d4', stroke: '#0d1117', strokeWidth: 2 }} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
