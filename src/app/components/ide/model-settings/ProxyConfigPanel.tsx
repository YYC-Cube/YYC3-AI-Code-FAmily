import { Check, CheckCircle2, Copy, FileCode2, Lightbulb, Lock, Network, RefreshCw, RotateCcw, Wifi, XCircle } from 'lucide-react';
...
const PROXY_SERVER_TEMPLATE = `export default { async fetch(request, env) {
  const corsHeaders = { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'POST, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Proxy-Auth' };
  if (request.method === 'OPTIONS') return new Response(null, { headers: corsHeaders });
  const auth = request.headers.get('X-Proxy-Auth');
  if (auth !== env.PROXY_AUTH_TOKEN) return new Response('Unauthorized', { status: 401 });
  const { provider, endpoint, method, body } = await request.json();
  const apiKey = env[provider.toUpperCase() + '_API_KEY'] || env[provider.toUpperCase() + '_API_KEY'];
  const headers = { 'Content-Type': 'application/json', Authorization: 'Bearer ' + apiKey };
  const url = env[provider.toUpperCase() + '_BASE_URL'] + endpoint;
...
function loadProxyConfig(): ProxyConfig {
  try {
    ...
    if (stored) return { ...DEFAULT_PROXY_CONFIG, ...JSON.parse(stored) };
  } catch { /* empty */ }
  return { ...DEFAULT_PROXY_CONFIG };
}

function saveProxyConfig(config: Partial<ProxyConfig>): ProxyConfig {
  const merged = { ...loadProxyConfig(), ...config };
  try { localStorage.setItem('yyc3_proxy_config', JSON.stringify(merged)); } catch { /* empty */ }
  return merged;
}

async function checkProxyHealth(baseUrl: string): Promise<{ healthy: boolean; latencyMs: number; version?: string; error?: string }> {
  const start = Date.now();
  try {
    const response = await fetch(`${baseUrl.replace(/\/+$/, "")}/health`, { method: "GET", signal: AbortSignal.timeout(5000) });
...
export function ProxyConfigPanel() {
  ...
  const handleHealthCheck = useCallback(async () => {
    ...
    try {
      ...
      setHealthError(e instanceof Error ? e.message : '检查失败');
    }
  }, [config.baseUrl]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[9px] text-white/20 bg-white/[0.03] px-1.5 py-0.5 rounded">{config.enabled ? '已启用' : '未启用'}</span>
      </div>

      <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 space-y-4" style={{ boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.02)' }}>
        <div className="flex items-center justify-between">
          <div>
            <div className="text-[11px] text-white/60">启用代理转发</div>
            <div className="text-[9px] text-white/20 mt-0.5">所有 LLM API 请求将通过代理服务器转发，解决 CORS 限制并保护 API Key</div>
          </div>
          <button onClick={() => handleSave({ enabled: !config.enabled })} className="shrink-0">
            <div className={`w-10 h-5 rounded-full transition-all ${config.enabled ? 'bg-indigo-500/40' : 'bg-white/[0.08]'}`}>
              <div className={`w-4.5 h-4.5 rounded-full transition-all mt-[1px] ${config.enabled ? 'bg-indigo-400 ml-[21px]' : 'bg-white/25 ml-[1px]'}`} style={{ width: '18px', height: '18px' }} />
            </div>
          </button>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex gap-2">
            <input value={config.baseUrl} onChange={e => handleSave({ baseUrl: e.target.value })}
            ...
            <button onClick={handleHealthCheck} disabled={healthStatus === 'checking'}
              ...
              {healthStatus === 'checking' ? <RefreshCw className="w-3 h-3 animate-spin" /> : <Wifi className="w-3 h-3" />} 健康检查
            </button>
          </div>
        ...
        <div className="space-y-1.5">
          <label className="text-[10px] text-white/30 uppercase tracking-wider"><Lock className="w-3 h-3 inline mr-1" />认证 Token (前端→代理)</label>
          <input type="password" value={config.authToken || ''} onChange={e => handleSave({ authToken: e.target.value })}
        ...
        {showTemplate && (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => { navigator.clipboard?.writeText(PROXY_SERVER_TEMPLATE); setTemplateCopied(true); setTimeout(() => setTemplateCopied(false), 2000); }}
            ...
            <pre className="text-[9px] text-white/30 font-mono bg-black/20 border border-white/[0.04] rounded-lg p-3 max-h-[200px] overflow-y-auto leading-relaxed whitespace-pre-wrap break-all">{PROXY_SERVER_TEMPLATE.trim()}</pre>
          </div>
        )}
      </div>
