import { Activity as ActivityIcon, ArrowRight, Bug, CheckCircle2, Clock, Cpu, Lightbulb, Loader2, RefreshCw, Wifi, XCircle } from 'lucide-react'
import { useMemo, useState } from 'react'
import type { ProviderDef } from '../constants/providers'
import { LatencyTrendChart } from './LatencyTrendChart'
import type { DiagnosticResult } from './types'

export function SmartDiagnosticsPanel({ providers, diagnostics, onRunDiagnostic, onSelectModel, activeModelKey }: {
    ...
    <div className="space-y-4">
        ...
        return (
          <div key={provider.id} className="space-y-1.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] text-white/15">{providerDiags.filter(d => d.diag.status === 'success').length}/{providerDiags.length} 在线</span>
            </div>
            {providerDiags.map(({ model, diag }) => {
              const modelKey = `${provider.id}:${model.id}`;
              const isActive = activeModelKey === modelKey;
              return (
                <div key={model.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all group ${isActive ? 'bg-indigo-500/[0.06] border border-indigo-500/20' : diag.status === 'success' ? 'bg-emerald-500/[0.03] border border-emerald-500/10 hover:border-emerald-500/20' : diag.status === 'error' ? 'bg-red-500/[0.03] border border-red-500/10' : 'bg-white/[0.01] border border-white/[0.04]'}`}>
                  {isActive ? <CheckCircle2 className="w-3.5 h-3.5 text-indigo-400 shrink-0" /> : diag.status === 'success' ? <CheckCircle2 className="w-3 h-3 text-emerald-400/60 shrink-0" /> : diag.status === 'error' ? <XCircle className="w-3 h-3 text-red-400/60 shrink-0" /> : <Loader2 className="w-3 h-3 text-cyan-400/60 animate-spin shrink-0" />}
                  <span className={`text-[10px] flex-1 ${isActive ? 'text-indigo-300' : 'text-white/50'}`}>{model.name}</span>
                  {isActive && <span className="text-[8px] px-1.5 py-0.5 rounded-full bg-indigo-500/20 text-indigo-400/80 border border-indigo-500/20 shrink-0">当前使用</span>}
                  ...
                  {diag.status === 'success' && !isActive && (
                    <button onClick={() => onSelectModel(provider.id, model.id)}
                      ...
                      <ArrowRight className="w-3 h-3" /> 选择使用
                    </button>
                  )}
                </div>
              );
      ...
      {errorModels > 0 && (
        <div className="rounded-xl border border-amber-500/15 bg-gradient-to-br from-amber-500/[0.04] to-orange-500/[0.02] p-4 space-y-2">
          <div className="flex items-center gap-2"><Lightbulb className="w-4 h-4 text-amber-400" /><span className="text-[12px] text-amber-400/80">AI 诊断建议</span></div>
          <div className="space-y-1.5 pl-6">
            {Object.values(diagnostics).filter(d => d.status === 'error').slice(0, 3).map((diag, i) => (
              <div key={i} className="text-[10px] text-white/35 flex items-start gap-1.5">
                ...
                <span><strong className="text-amber-400/50">{diag.modelName}</strong>: {diag.message.includes('401') ? '请检查 API Key 是否正确配置且未过期' : diag.message.includes('429') ? '请求频率超限，建议稍后重试或升级配额' : diag.message.includes('网络') || diag.message.includes('fetch') ? '网络连接失败，请确认端点 URL 是否可达' : diag.message.includes('超时') ? '连接超时，可能是网络不稳定或服务暂时不可用' : '请检查配置或查看错误详情'}</span>
              </div>
            ))}
          </div>
        </div>
