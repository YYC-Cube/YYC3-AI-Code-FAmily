...
class APIKeyVault {
  ...
  async getKey(id: string): Promise<string | null> {
    ...
    const config: APIKeyConfig | undefined = await this.db!.get(STORE_NAME, id);
  ...
  async listKeys(): Promise<APIKeyConfig[]> {
    ...
    const keys: APIKeyConfig[] = await this.db!.getAll(STORE_NAME);
