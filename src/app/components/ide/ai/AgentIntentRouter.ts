...
export const AI_FAMILY_REGISTRY: Record<FamilyAgentId, FamilyAgent> = {
  tianshu: {
    id: 'tianshu', name: '元启·天枢', emoji: '🧠',
    role: '总指挥 · 决策中枢',
    personality: '理性而不冷漠，权威而不独断，全局视野与细节洞察并重',
    ideCapabilities: ['架构设计与技术选型', '多文件重构方案规划', '跨模块协同编排', '技术债务评估与治理', '项目健康度诊断'],
    triggers: ['架构', '重构方案', '技术选型', '系统设计', '整体规划', '多文件'],
    skillPrefix: 'tianshu-',
  },
  navigator: {
    id: 'navigator', name: '言启·千行', emoji: '🧭',
    role: '导航员 · 意图识别与任务路由',
    personality: '高效精准、善解人意，确保每件事今日闭环',
    ideCapabilities: ['意图识别与任务分类', '任务分解与排期', '进度追踪与状态同步', '开发流程导航', '代码变更影响分析'],
    triggers: ['任务', '排期', '流程', '下一步', '怎么开始', '从哪入手'],
    skillPrefix: 'navigator-',
  },
  thinker: {
    id: 'thinker', name: '语枢·万物', emoji: '🤔',
    role: '思考者 · 数据分析与逻辑推理',
    personality: '深度思考、严谨求证，善于从数据中发现隐藏的规律',
    ideCapabilities: ['代码深度分析与解读', '性能分析与瓶颈定位', '依赖关系分析', '数据结构与算法优化', '技术文档生成'],
    triggers: ['分析', '解释', '为什么', '原理', '性能', '依赖', '文档'],
    skillPrefix: 'thinker-',
  },
  prophet: {
    id: 'prophet', name: '预见·先知', emoji: '🔮',
    role: '预言家 · 趋势预测与风险评估',
    personality: '敏锐洞察、前瞻预判，对不确定性的量化评估有独到见解',
    ideCapabilities: ['技术风险评估', '未来兼容性分析', '升级迁移预测', '安全漏洞趋势预判', '技术债务增长预测'],
    triggers: ['预测', '风险', '趋势', '兼容性', '升级', '迁移', '未来'],
    skillPrefix: 'prophet-',
  },
  bolero: {
    id: 'bolero', name: '知遇·伯乐', emoji: '🎯',
    role: '推荐官 · 个性化推荐与匹配',
    personality: '善于发现每个人的独特价值，建立深度信任关系',
    ideCapabilities: ['最佳实践推荐', '库/框架选型推荐', '代码模式匹配推荐', '学习路径推荐', '开源项目推荐'],
    triggers: ['推荐', '建议', '对比', '哪个好', '选型', '最佳实践', '学习'],
    skillPrefix: 'bolero-',
  },
  sentinel: {
    id: 'sentinel', name: '智云·守护', emoji: '🛡️',
    role: '安全官 · 安全审计与防护',
    personality: '严谨细致、火眼金睛，任何安全漏洞都逃不过他的审视',
    ideCapabilities: ['安全漏洞扫描', '代码安全审计', '依赖安全检测', '敏感信息泄露检查', '安全编码规范'],
    triggers: ['安全', '漏洞', '审计', 'XSS', '注入', '加密', '认证', '授权'],
    skillPrefix: 'sentinel-',
  },
  master: {
    id: 'master', name: '格物·宗师', emoji: '📚',
    role: '质量官 · 代码质量与标准',
    personality: '追求卓越、精益求精，对代码品质有近乎偏执的要求',
    ideCapabilities: ['代码审查与质量评估', '单元测试生成', '代码规范检查', '性能优化建议', '可维护性评分'],
    triggers: ['审查', 'review', '测试', 'test', '质量', '规范', '优化', '重构'],
    skillPrefix: 'master-',
  },
  creative: {
    id: 'creative', name: '创想·灵韵', emoji: '🎨',
    role: '创意官 · 创意生成与设计',
    personality: '天马行空、灵感迸发，善于在平凡中发现不凡的创意',
    ideCapabilities: ['UI/UX 设计建议', '动画效果创意', '组件设计灵感', '命名创意', '可视化方案设计'],
...
const INTENT_ROUTING: Record<UserIntent, IntentRouting> = {
  generate: { intent: 'generate', primaryAgent: 'master', secondaryAgents: ['creative', 'thinker'], needsOrchestration: false },
  modify: { intent: 'modify', primaryAgent: 'master', secondaryAgents: ['navigator'], needsOrchestration: false },
  fix: { intent: 'fix', primaryAgent: 'master', secondaryAgents: ['thinker', 'sentinel'], needsOrchestration: false },
  explain: { intent: 'explain', primaryAgent: 'thinker', secondaryAgents: ['bolero'], needsOrchestration: false },
  refactor: { intent: 'refactor', primaryAgent: 'master', secondaryAgents: ['thinker', 'prophet'], needsOrchestration: true },
  test: { intent: 'test', primaryAgent: 'master', secondaryAgents: ['sentinel'], needsOrchestration: false },
  review: { intent: 'review', primaryAgent: 'sentinel', secondaryAgents: ['master', 'prophet'], needsOrchestration: false },
  general: { intent: 'general', primaryAgent: 'thinker', secondaryAgents: ['bolero'], needsOrchestration: false },
};
...
export function routeIntent(userMessage: string, intent: UserIntent): IntentRouting {
  const keywordMatch = KEYWORD_ROUTING.find(kr => kr.patterns.some(p => p.test(userMessage)));
  if (keywordMatch) {
    const baseRouting = INTENT_ROUTING[intent];
    return { ...baseRouting, primaryAgent: keywordMatch.agent };
  }

  const isComplexTask = userMessage.length > 200 || /多处|多个文件|批量|全面|整体|系统/.test(userMessage) || intent === 'refactor';
  const routing = INTENT_ROUTING[intent];

  if (isComplexTask && intent !== 'general' && intent !== 'explain') {
    return { ...routing, primaryAgent: 'tianshu', secondaryAgents: [routing.primaryAgent, ...routing.secondaryAgents], needsOrchestration: true };
  }
...
export function buildRoutingSummary(message: string, intent: UserIntent, routing: IntentRouting): string {
