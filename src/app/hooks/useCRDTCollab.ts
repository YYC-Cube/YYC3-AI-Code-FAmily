 ...
 * description: CRDT 实时协同 hook v2.0.0 — 基于 yjs + 自定义 WebSocket Provider + Awareness 协议，实现多人实时代码编辑、光标/选区感知、IndexedDB 持久化、断线重连、状态追踪
...
class YWebSocketProvider {
  ...
  private sendSyncStep1() {
    ...
    this.send(WSMessageType.SYNC_STEP1, sv.buffer as ArrayBuffer);
  }

  private sendUpdate(update: Uint8Array) {
    this.send(WSMessageType.SYNC_UPDATE, update.buffer as ArrayBuffer);
  }
    ...
    try {
      ...
      switch (type) {
        ...
        case WSMessageType.SYNC_STEP1: {
          ...
          this.send(WSMessageType.SYNC_STEP2, update.buffer as ArrayBuffer);
          break;
        }
      }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
