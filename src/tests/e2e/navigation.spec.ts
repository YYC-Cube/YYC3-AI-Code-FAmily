 ...
 * updated: 2026-04-04
 * status: stable
 * tags: testing,e2e,playwright,navigation
 * priority: P0
 */
...
test.describe('跨路由 Bridge 数据传递', () => {
  ...
  test('TC-BRG-E2E-002: Designer 发送设计 → AI Code 接收代码', async ({ page: _page }) => {
    // 类似流程，反向
  });
});
...
test.describe('主题切换一致性', () => {
  ...
  test('TC-THEME-001: Designer 切换主题后 AI Code 同步', async ({ page: _page }) => {
...
test.describe('错误边界 E2E', () => {
  ...
  test('TC-EB-E2E-001: 路由级 ErrorBoundary 捕获页面崩溃', async ({ page: _page }) => {
    // 注入一个会崩溃的状态
    // 验证看到错误页面而不是白屏
  });

  test('TC-EB-E2E-002: 点击"重新加载"可恢复', async ({ page: _page }) => {
    // 在错误页面点击重新加载按钮
    // 验证恢复正常
  });

  test('TC-EB-E2E-003: 点击"返回首页"可导航', async ({ page: _page }) => {
    // 在错误页面点击返回首页
    // 验证回到 /
  });
});
