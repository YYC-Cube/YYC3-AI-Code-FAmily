...
import {
  GripHorizontal, X, Maximize2, SquareSplitHorizontal, Copy, MoreHorizontal,
  Table, ChartBar, TrendingUp, TextCursorInput, RectangleHorizontal,
  Play, GitBranch, FileText, Code, List, Square, ChevronDown, Sparkles,
  Plus, Columns, Rows, Eye, LayoutGrid, FileCode2, FormInput, Layers,
  Merge, ArrowRightLeft, ArrowUpDown, Scissors, ArrowLeft, Magnet, Move,
  Group, Ungroup, Lock, Unlock, MousePointer2
} from 'lucide-react';
import { useDesigner, type ComponentDef, type Panel as PanelType, type ComponentInstance } from '../../store';
...
function useAccentColors(t: ReturnType<typeof useThemeTokens>) {
  ...
  return {
    isAurora,
    isLiquid,
    raw: isAurora ? '#00ff87' : isLiquid ? '#8b5cf6' : '#6366f1',
    rawRgb: isAurora ? '0,255,135' : isLiquid ? '139,92,246' : '99,102,241',
    secondary: isAurora ? '#60efff' : isLiquid ? '#d946ef' : '#a855f7',
    secondaryRgb: isAurora ? '96,239,255' : isLiquid ? '217,70,239' : '168,85,247',
    checkbox: isAurora ? '#00ff87' : isLiquid ? '#8b5cf6' : '#6366f1',
...
function generateIframeTableHTML(c: { id: string; props: Record<string, any> }): string {
  const cols: string[] = c.props.columns || DEFAULT_COLUMNS;
  ...
  const thChk = [
    ...
    '<input type="checkbox" id="chkAll_' + T + '" style="accent-color:#6366f1;cursor:pointer"',
  ...
  const tbHTML = mockRows.map(row => {
    ...
    return '<tr class="tr_' + T + '" style="border-bottom:1px solid rgba(255,255,255,0.03)"><td style="padding:6px 4px;text-align:center"><input type="checkbox" class="rc_' + T + '" onchange="updB_' + T + '()" style="accent-color:#6366f1;cursor:pointer"/></td>' + tds + '</tr>';
  }).join('');
  const batchBar = [
...
function generateIframeHTML(comps: ComponentInstance[]): string {
  const compHTML = comps.map(c => {
    switch (c.type) {
      case 'Button':
        return `<button style="padding:8px 16px;border-radius:8px;background:#6366f1;color:#fff;border:none;font-size:13px;cursor:pointer">${c.props.label || '按钮'}</button>`;
      case 'Input':
      ...
      case 'Chart':
        ...
        const isBar = c.props.chartType === 'bar';
      ...
      /* DEAD_CODE_BLOCK_WAS_HERE
        ...
        const thAllChk = '<th style="padding:8px 4px;text-align:center;border-bottom:1px solid rgba(255,255,255,0.06)"><input type="checkbox" id="chkAll_' + tblId + '" style="accent-color:#6366f1;cursor:pointer" onchange="document.querySelectorAll(\'.rc_' + tblId + '\').forEach(function(c){c.checked=this.checked}.bind(this));updBatch_' + tblId + '()"/></th>';
        ...
        const tbodyHTML = mockRows.map((row, ri) => '<tr class="tr_' + tblId + '" style="border-bottom:1px solid rgba(255,255,255,0.03)"><td style="padding:6px 4px;text-align:center"><input type="checkbox" class="rc_' + tblId + '" onchange="updBatch_' + tblId + '()" style="accent-color:#6366f1;cursor:pointer"/></td>' + row.slice(0, cols.length).map((cell: string) => '<td style="padding:6px 12px;color:rgba(255,255,255,0.6);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + renderCell(cell) + '</td>').join('') + '</tr>').join('');
      ...
      case 'Progress':
        return `<div><div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-size:12px;color:rgba(255,255,255,0.4)">进度</span><span style="font-size:12px;color:rgba(255,255,255,0.5)">${c.props.value}%</span></div><div style="height:8px;background:rgba(255,255,255,0.06);border-radius:999px;overflow:hidden"><div style="height:100%;width:${c.props.value}%;background:#6366f1;border-radius:999px"></div></div></div>`;
      case 'Checkbox':
        return `<label style="display:flex;align-items:center;gap:8px;font-size:13px;color:rgba(255,255,255,0.6);cursor:pointer"><input type="checkbox" style="accent-color:#6366f1" />${c.props.label || '复选项'}</label>`;
      case 'Switch':
        return `<label style="display:flex;align-items:center;gap:8px;font-size:13px;color:rgba(255,255,255,0.6);cursor:pointer"><span>${c.props.label || '开关'}</span><div style="width:36px;height:20px;border-radius:999px;background:rgba(255,255,255,0.1);position:relative"><div style="width:16px;height:16px;border-radius:50%;background:#6366f1;position:absolute;top:2px;left:2px"></div></div></label>`;
      default:
...
const DEFAULT_COLUMNS = ['Name', 'Email', 'Role'];
...
function ComponentRenderer({ comp }: { comp: { id: string; type: string; props: Record<string, any>; groupId?: string } }) {
  const { selectComponent, selectedComponentId, toggleSelectComponent, selectedComponentIds, components, ungroupComponent, groupComponents } = useDesigner();
  const t = useThemeTokens();
  ...
  const renderContent = () => {
    ...
    switch (comp.type) {
      ...
      case 'Table': return <MockTable columns={comp.props.columns || DEFAULT_COLUMNS} />;
      case 'Button':
...
function PanelCard({ panel }: { panel: PanelType }) {
  const {
    ...
    crdtPeers, currentUserIdentity, lockPanel, unlockPanel
  } = useDesigner();
  const t = useThemeTokens();
  const ac = useAccentColors(t);
...
function SubCanvasMode() {
  const { subCanvasPanelId, exitSubCanvas, panels, components, selectComponent, addComponentToPanel, selectedComponentId } = useDesigner();
  const t = useThemeTokens();
  const ac = useAccentColors(t);
